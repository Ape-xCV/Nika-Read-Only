...
default_models=(
  "CT250MX500SSD1"
  "CT240BX500SSD1" "EMTCE X150 240GB"
  "INTENSO SSD"    "KINGSTON SA400S37240G"
...
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/istatic void smbios_build_type_7_table(void)"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i{"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    SMBIOS_BUILD_TABLE_PRE(7, T7_BASE, true); /* required */"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    t->cache_configuration = 0b0000000100000010; // 000000 WriteBack(01) Disabled(0) Internal(00) 0 NotSocketed(0) L3(010)"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    t->maximum_cache_size = 0x8400;              // 64 MiB"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    t->installed_size = 0x8400;"
...
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    t->maximum_cache_size_2 = 0x80000400;        // 64 MiB"
sed -i "$file_smbios" -Ee "/static void smbios_build_type_8_table\(void\)/i\    t->installed_size_2 = 0x80000400;"
...
sed -i "$file_smbios" -Ee "/static void smbios_build_type_32_table\(void\)/i\    t->partition_row_position = 0x01; /* Single row */"
...
echo "    SMBIOS_SET_DEFAULT(type7.socket_designation, \"CACHE0\");"
sed -i "$file_smbios" -Ee "/    SMBIOS_SET_DEFAULT\(type4.version, version\);/a\    SMBIOS_SET_DEFAULT(type7.socket_designation, \"CACHE0\");"
...
echo "    smbios_build_type_7_table();"
echo "    ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^"
echo "    smbios_build_type_8_table();"
sed -i "$file_smbios" -Ee "/    smbios_build_type_8_table\(\);/i\    smbios_build_type_7_table();"
...
echo "        smbios_build_type_17_table(i, GET_DIMM_SZ);"
echo "        v v v v v v v v v v v v v v v v v v v v v v"
echo "        smbios_build_type_20_table(i, GET_DIMM_SZ);"
sed -i "$file_smbios" -Ee "/        smbios_build_type_17_table\(i, GET_DIMM_SZ\);/a\        smbios_build_type_20_table(i, GET_DIMM_SZ);"
echo "    if (slot_cnt > dimm_cnt)"
