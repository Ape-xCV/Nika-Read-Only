...
sed -i "$file_smbios" -Ee "/static void smbios_build_type_32_table\(void\)/i\    t->temperature_probe_handler = 0xFFFF;  // no thermal probe"
...
sed -i "$header_smbios" -Ee "/\/\* SMBIOS type 32 - System Boot Information \*\//i\    uint16_t temperature_probe_handler;"
