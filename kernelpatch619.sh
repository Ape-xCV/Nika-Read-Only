...
if [[ "${cpu_vendor:1}" == "AuthenticAMD" ]]; then
  cp -f "amd619.mypatch" "${TKG_DIR}/linux${KERNEL_MAJOR}${KERNEL_MINOR}-tkg-userpatches/."
else
  cp -f "intel619.mypatch" "${TKG_DIR}/linux${KERNEL_MAJOR}${KERNEL_MINOR}-tkg-userpatches/."
fi
