...
if [[ "${cpu_vendor:1}" == "AuthenticAMD" ]]; then
  cp -f "amd618.mypatch" "${TKG_DIR}/linux${KERNEL_MAJOR}${KERNEL_MINOR}-tkg-userpatches/."
else
  cp -f "intel618.mypatch" "${TKG_DIR}/linux${KERNEL_MAJOR}${KERNEL_MINOR}-tkg-userpatches/."
fi
